...
def initParams(
    params, inner_group_size, outer_group_size, n_layer, residuals_per_layer, rand_key
):
    ...
    for name, param in flat_params.items():
        ...
        if name[0] == "blocks" and "mixer" in name:  # Mamba block w/o norm stuff
            if "bias" in name and "dt_proj" not in name:
                ...
                if "out_proj" in name:  # 1.
                    ...
                    param = random.uniform(used_key, shape, minval=-value, maxval=value)
                    param = param.astype(param.dtype)
        ...
        elif name[0] == "blocks" and (
            "norm" in name or "out_norm" in name
        ):  # Mamba block norm stuff

            print(f"Skipping {name}")

        elif (
            name[0] == "encoder"
            or name[0] == "propagation_0"
            or "post_layers" in name[0]
            or "label_conv" in name[0]
        ):  # Encoder and final PointNet layers are only convs
            ...
            if "kernel" in name:
                ...
                if tuple(bias_name) in flat_params:
                    ...
                    # initialize bias
                    rand_key, used_key = random.split(rand_key)
                    flat_params[tuple(bias_name)] = random.uniform(
                        used_key, bias_shape, minval=-value, maxval=value
                    )
                else:
                ...
                print(
                    f"Skipping {name} and its bias, as they are associated with norm layers."
                )
